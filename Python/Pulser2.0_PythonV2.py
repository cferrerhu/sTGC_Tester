...
while(root.winfo_exists()):
    ...
    # impossible choice
        ...
        print("This does not exist, try another combination!")
        Beep.finished()
        root.mainloop()
    ...
    # test the first three USB ports
    try:
        portName = serial.Serial('/dev/ttyUSB0', bps)
    except serial.serialutil.SerialException:
        try:
            portName = serial.Serial('/dev/ttyUSB1', bps)
        except serial.serialutil.SerialException:
            try:
                portName = serial.Serial('/dev/ttyUSB2', bps)
            except serial.serialutil.SerialException:
                print("Arduino not connected")
    
    # opens selection window for com ports
    if debug3:
        if len(portName) < 3:
            ports = serial_ports()
    ...
    if print_CSV:
        ...
        # Excel setup
        outputDir = datetime.now().strftime("%Y_%b_%d_sTGC_"+selected_map_filename+"_"+selected_board)
        ...
        try:
            os.system('mkdir '+'Results/'+selected_map_filename[:-7] + ' > /dev/null 2>&1')
        except:
            pass
        workbook = xlsxwriter.Workbook('Results/'+selected_map_filename[:-7]+'/'+outputDir + '.xlsx')
        ...
        prueba.write(0, 4, selected_map_filename)
        ...
        errores.write(0, 1, selected_map_filename)
        ...
        conectorDC.write(2, 1, selected_map_filename, white)
        ...
        conectorAC.write(2, 1, selected_map_filename, white)
